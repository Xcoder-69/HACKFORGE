# 🌾 AgroMind AI — Application Structure & Technical Specification
...
This document provides a simple, structured explanation of the core technical pillars powering **AgroMind AI**:
1. [🔄 Complete Application Flow & Feature Journey](#1--complete-application-flow--feature-journey)
2. [🤖 AI Models & Vision Architecture](#2--ai-models--vision-architecture)
3. [🔐 Authentication & Session Security](#3--authentication--session-security)
4. [🧠 Trained Knowledge & Deterministic Agronomic Engines](#4--trained-knowledge--deterministic-agronomic-engines)
5. [📍 Geolocation & All-India Administrative Hierarchy](#5--geolocation--all-india-administrative-hierarchy)
6. [☁️ Cloud Database & Offline-First Sync Architecture](#6--cloud-database--offline-first-sync-architecture)
7. [📊 Complete Feature-by-Feature Matrix](#7--complete-feature-by-feature-matrix)

---

## 1. 🔄 Complete Application Flow & Feature Journey
...
flowchart TD
    ...
    subgraph Onboarding ["4-Step Onboarding Wizard (/onboarding)"]
        Onboard --> Step1["Step 1: KYC & PM-KISAN ID"]
        Step1 --> Step2["Step 2: 1-Tap GPS Farm Tagging"]
        Step2 --> Step3["Step 3: Land Acreage & Soil Classification"]
        Step3 --> Step4["Step 4: Seasonal Crop & Stage Setup"]
    end
    Step4 --> Home
    ...
    subgraph CoreHub ["🌾 Farmer Application Ecosystem (/home)"]
        ...
        Home --> Telemetry["Live Microclimate & IoT Telemetry"]
        
        ParcelMap --> Cam["📸 Crop Disease Leaf Scanner\n(/ai-camera)"]
        ParcelMap --> Recs["🌱 Crop Intelligence & Stages\n(/recommendations)"]
        ParcelMap --> Wx["🌦️ 7-Day Weather & Soil Probes\n(/weather-soil)"]
        ParcelMap --> Mkt["📈 APMC Mandi Market Prices\n(/market)"]
        ParcelMap --> Fin["💰 Expense & Profit Accounting\n(/expenses & /profit)"]
        ParcelMap --> Tasks["✅ Action Center & Checklist\n(/alerts)"]
        ParcelMap --> Chat["💬 Conversational AI Agronomist\n(/ai-assistant)"]
    end

    %% KVK Admin
    subgraph AdminHub ["🏛️ Extension Officer Command Center (/admin)"]
...
1. **Welcome & Localization (`/`)**: Farmer chooses their preferred language (**Gujarati**, **Hindi**, or **English**). The selection immediately adapts all terminology, audio tags, and number formatting.
...
4. **My Farm Dashboard (`/home`)**: The primary command deck featuring:
   ...
   - **Interactive Parcel Map**: Displays dynamic plot blocks (`Block A`, `Block B`, `Block C`...) with individual crop stages, soil moisture, and active glowing indicators.
...
6. **Mandi Market Rates (`/market`)**: Real-time prices across major APMC markets in Gujarat & India, with trend graphs and "Sell vs. Store" recommendations.
7. **Expense & Profit Accounting (`/expenses`, `/profit`)**: Simple digital ledger tracking seeds, fertilizer, machinery, and labor costs to calculate the farmer's **Break-Even Price** and expected net profit.
...
## 2. 🤖 AI Models & Vision Architecture
...
- **Why Gemini 1.5 Flash?**: Ultra-low latency (< 1.8 seconds), native multimodal token processing, high precision on plant pathology, and cost-effective token consumption.
- **Data Pipeline**:
...
sequenceDiagram
    ...
    alt Standard Live Verification
        UI->>FB: Requests Phone Auth reCAPTCHA & Dispatches OTP
    ...
    alt Profile Exists
        DB-->>UI: Returns User Profile & Active Farm Parcel
...
## 4. 🧠 Trained Knowledge & Deterministic Agronomic Engines
...
flowchart TD
    ...
    subgraph Rules ["Local Rule Taxonomies"]
        PestRule["Pest & Insect Heuristics\n(Pink Bollworm, Whitefly, Aphids)"]
        WaterRule["Irrigation & Soil Moisture Matrix\n(Moisture < 40% vs. Rain Forecast)"]
        FertRule["Nutrient & Growth Stage Guide\n(Urea vs. Flowering Stage Rules)"]
...
1. **Pest & Pathogen Heuristics**:
   - *Pink Bollworm in Cotton*: Detects keyword triggers $\rightarrow$ prescribes Gossyplure pheromone traps (5/acre) and Emamectin Benzoate 5% SG @ 5g/10L.
   - *Tikka Disease in Groundnut*: Identifies leaf spot symptoms $\rightarrow$ recommends Mancozeb 75% WP or Chlorothalonil 75% WP.
2. **Moisture & Irrigation Thresholds**:
   - Soil moisture $> 70\%$ + rain forecasted within 48h $\rightarrow$ **Action**: *"Postpone drip irrigation for 24–48 hours to avoid root rot."*
   - Soil moisture $< 35\%$ during flowering stage $\rightarrow$ **Action**: *"Critical moisture stress detected. Run micro-drip cycle tomorrow 06:30 AM."*
3. **Fertilizer Dosage & Stage Timing**:
   - Sowing stage $\rightarrow$ Basal application of DAP and Potash.
   - Vegetative to Flowering stage $\rightarrow$ Split-dose Urea application, strictly withholding nitrogen within 36 hours of forecasted rainfall.

---

## 5. 📍 Geolocation & All-India Administrative Hierarchy
...
flowchart LR
    subgraph GeoInputs ["Dual Geolocation Inputs"]
        GPS["🛰️ HTML5 Geolocation API\nLat, Lng, Accuracy"]
        DIR["🏛️ All-India Cascading Hierarchy\n28 States & 8 UTs"]
    end

    subgraph Processing ["Location Processor"]
        CAS["Cascading Selector:\nState -> District -> City -> Taluka/Village"]
        BOUND["Geo-bounding & Hyperlocal Matching"]
    end
...
- **28 States & 8 Union Territories** included in a structured cascading tree.
- **Deep Gujarat Coverage**: All 33 districts mapped with their corresponding cities, talukas, and primary village clusters:
  - *Surat* $\rightarrow$ Kamrej, Olpad, Bardoli, Mandvi, Mangrol...
  - *Rajkot* $\rightarrow$ Gondal, Jetpur, Dhoraji, Jasdan, Kotda Sangani...
  - *Ahmedabad* $\rightarrow$ Sanand, Dholka, Bavla, Viramgam, Mandal...
...
## 6. ☁️ Cloud Database & Offline-First Sync Architecture
...
```mermaid
flowchart TD
    UserAction["Farmer performs action\n(e.g., adds Block C, logs ₹1,200 expense)"] --> L1Cache
...
| **Crop Intelligence** | `/recommendations` | Agronomic Suitability Matrix | Soil class, irrigation availability, season | Recommended crops ranked by profit margin & water need |
| **Weather & Soil Telemetry** | `/weather-soil` | Open-Meteo Meteorological API | Farm GPS Latitude & Longitude | 7-day hourly rain graphs, humidity, temperature warnings |
| **Crop Leaf Scanner** | `/ai-camera` | **Google Gemini 1.5 Flash (Vision)** | Mobile camera leaf photo (JPEG/PNG) | Disease name, severity %, organic & chemical remedies |
...
| **Profit & Yield Overview** | `/profit` | Unit Economics Engine | Harvest quantity (Qtl), realized sale price | Net profit/loss, return on investment, break-even rate |
| **Action Center & Alerts** | `/alerts` | Autonomous Action Orchestrator | Weather threats, scan diagnoses, moisture levels | Prioritized daily checklist with completion checkboxes |
...
- **Dynamic Farm & Plot Service**: [src/services/farmService.ts](file:///c:/Users/mahen/OneDrive/Desktop/HACKFORGE/src/services/farmService.ts)
...
<div align="center">
  <sub>🌾 AgroMind AI — Technical Architecture & Feature Specification. Bit N Build '26 Gujarat.</sub>
</div>
