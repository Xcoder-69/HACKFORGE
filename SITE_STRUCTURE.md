# AgroMind AI — Website and Application Structure
...
flowchart TD
    ...
    subgraph FarmerApp ["Farmer Portal (Protected by FarmerLayout)"]
        G[Home and Farm Dashboard: /home]
        H[Crop Recommendations: /recommendations]
        I[Weather and Soil Telemetry: /weather-soil]
        J[AI Camera Disease Scanner: /ai-camera]
        K[Mandi Market Prices: /market]
        L[Farm Expense Tracker: /expenses]
        M[Profit and Yield Overview: /profit]
        N[Alerts and Action Center: /alerts]
        O[AI Agronomist Chat: /ai-assistant]
        P[Farmer Profile and Settings: /profile]
    end
    ...
    subgraph AdminPortal ["KVK Extension Officer Command Center"]
        E[Admin Dashboard: /admin]
        E1[Farmer Directory and Telemetry]
    ...
    %% Backend Services
    subgraph CloudBackend ["Cloud and Offline Backend Stack"]
        DB[(Supabase PostgreSQL: 13 Live Tables)]
        FB[Google Firebase Real SMS OTP]
        WX[Open-Meteo Live Weather Telemetry]
        SE[Sync Engine and LocalStorage Cache]
    end

    FarmerApp -.->|Offline-First and Live Sync| CloudBackend
    AdminPortal -.->|Real-Time Analytics| DB
```

---

## 1. Authentication and Onboarding Layer

| Route | Page / Screen | Description |
| :--- | :--- | :--- |
| `/` | **Welcome and Language Selection** | Initial landing screen. Choose Gujarati (ગુજરાતી), Hindi (हिन्दी), or English. |
| `/login` | **Unified Login and Registration** | **Tab 1 (Login):** Enter registered phone number + Firebase 6-digit real SMS OTP or demo PIN `8249`.<br>**Tab 2 (Register):** Full Name, Phone, and 3-Tier Cascading District -> City -> Village selector. |
| `/onboarding` | **4-Step Farm Setup Wizard** | Completed once after initial registration:<br>1. KYC and Location verification<br>2. GPS farm geolocation tagging<br>3. Land acreage, soil type, and irrigation source<br>4. Seasonal crop selection |

---

## 2. Farmer Core Application (`/home` — Protected)
Wrapped in responsive **`FarmerLayout`** (collapsible desktop sidebar on >= 1024px; top header, compact slide-over drawer, and bottom navigation bar on mobile).

| Route | Screen Name | Key Features |
| :--- | :--- | :--- |
| `/home` | **My Farm Dashboard** | Farm telemetry overview, weather widget, Plot A, B, C progress trackers, quick action shortcuts. |
...
| `/weather-soil` | **Weather and Soil Telemetry** | Real-time Open-Meteo GPS data, 7-day hourly rainfall forecast, temperature, humidity, and agro-advisories. |
...
| `/market` | **Mandi Market Rates** | Live APMC mandi crop prices across Gujarat and India, modal prices, daily trends, and "Sell Now vs. Hold" recommendations. |
...
| `/profit` | **Profit and Yield Overview** | Crop harvest records, market revenue vs. input expenses, net income calculation. |
| `/alerts` | **Alerts and Action Center** | Urgent weather alerts, pest infestation warnings, irrigation reminders, and actionable field tasks. |
| `/ai-assistant` | **Conversational AI Agronomist** | Multilingual chat advisory powered by contextual farm and crop parameters. |
| `/profile` | **Farmer Account and Settings** | Edit farmer details, PM-KISAN linking ID, security PIN updates, and notification toggles. |
...
## 4. Backend and Data Integration Layer
...
- **Open-Meteo Weather API**: Live, GPS-accurate hourly atmospheric and soil telemetry without API key limits.
